...
export const ChatItem = ({ id, content, member, timestamp, fileUrl, deleted, currentMember, isUpdated, socketUrl, socketQuery }: ChatItemProps) => {
	...
	// Cancel message editing on 'esc' key press
	useEffect(() => {
	...
	// Function to handle member clicks
	const onMemberClick = () => {
		// Return if member clicked on himself
				...
				<div className="flex flex-col w-full">
					...
					{isPDF && (
						<div className="relative p-2 mt-2 rounded-md bg-background/10">
							<a
								...
								className="text-sm text-zinc-500 dark:text-zinc-400 hover:underline flex items-center"
							>
					...
					{!fileUrl && !isEditing && (
						<p className={cn("text-sm text-zinc-600 dark:text-zinc-300", deleted && "italic text-zinc-500 dark:text-zinc-400 text-xs mt-1")}>
							{content}
							{isUpdated && !deleted && <span className="text-[10px] mx-2 text-zinc-500 dark:text-zinc-400">(modifié)</span>}
						</p>
					)}
					{!fileUrl && isEditing && (
						<Form {...form}>
							...
							<span className="text-[10px] mt-1 text-zinc-400">Appuyez sur Esc pour Annuler, ou Enter pour Enregistrer</span>
						</Form>
					)}
				</div>
			</div>
